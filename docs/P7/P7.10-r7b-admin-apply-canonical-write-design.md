...
|---|--------------|-------|-------|
| 1 | **P7.10-r7a** dry-run canonical write payload builder (rehearsal lane) | Eng | Code allowed; **no DB write** — optional parallel to sidecar lane |
| 2 | [P7.10-r7e](./P7.10-r7e-canonical-apply-rollback-snapshot-design.md) rollback snapshot design | Eng + Ops | **DONE** (design only) |
| 3 | [P7.10-r7d](./P7.10-r7d-admin-apply-preview-payload-builder-closeout.md) apply preview builder (pure) | Eng | **DONE** — **no** DB / MatchResult write |
| 4 | **P7.10-r7f** `GET …/apply-preview` API wiring | Eng | **BLOCKED** — read-only HTTP |
| 5 | **P7.10-r7g** rollback snapshot builder dry-run | Eng | **BLOCKED** |
| 6 | **P7.10-r7h** Admin apply review UI design | Eng | **BLOCKED** |
| 7 | **P7.10-r8** Apply + rollback implementation | Eng | **BLOCKED** — first `MatchResult` write milestone |
| 8 | **P7.6-r9e3c** prod/staging-prod-like read-only evidence | Ops | [r9e3b access package](./P7.6-r9e3b-real-prod-reverify-access-package-and-ops-handoff.md) |
| 9 | PM/Ops written signoff for cohort + blast radius | PM/Ops | Per viewer / `sourceVersion` |
| 10 | Monitoring + kill switch ready | Ops | Grafana P0 — [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) **pending** |
| 11 | [P7.10-r7](./P7.10-r7-canonical-write-readiness-design.md) gates **12–20** PASS | Program | No partial prod Apply |

**r7d / r7e alone do not authorize Apply, rollback execution, or `MatchResult` promotion.**

**Environment rule:**

| r9e3 state | Apply allowed environment |
|------------|---------------------------|
| `NEED_MORE_PROD_REVERIFY` | **none** for MatchResult mutation — **r8** blocked; local preview via **r7f** only after unblocked |
...
## 13. Implementation phases (aligned with [r7c §14](./P7.10-r7c-admin-apply-canonical-promotion-api-design.md))

| Id | Milestone | Writes DB / MatchResult? | Status |
|----|-----------|--------------------------|--------|
| **r7b** | Admin Apply UX design (this doc) | **no** | **DONE** (design only) |
| **r7c** | [Promotion API design](./P7.10-r7c-admin-apply-canonical-promotion-api-design.md) | **no** | **DONE** (design only) |
| **r7d** | [Apply preview payload builder](./P7.10-r7d-admin-apply-preview-payload-builder-closeout.md) | **no** | **DONE** |
| **r7e** | [Rollback snapshot design](./P7.10-r7e-canonical-apply-rollback-snapshot-design.md) | **no** | **DONE** (design only) |
| **r7a** | Rehearsal dry-run payload builder (parallel track) | artifact only | **BLOCKED** |
| **r7f** | `GET …/apply-preview` API wiring | **no** (read-only) | **BLOCKED** |
| **r7g** | Rollback snapshot builder dry-run | **no** until r8 | **BLOCKED** |
| **r7h** | Admin apply review UI design | **no** | **BLOCKED** |
| **r8** | Apply + rollback + migration | **yes** | **BLOCKED** until Gate 12 / Grafana / signoff |
| — | Staging-prod-like promotion smoke | — | **BLOCKED** |
| — | PM/Ops signoff · production cohort proposal | — | **PENDING** / **BLOCKED** |

**Dependency graph:**

```text
r7b (UX design) → r7c (API design)
  → r7d (preview builder, no-write)     [DONE]
  → r7e (rollback snapshot design)      [DONE]
  → r7f (GET apply-preview, read-only)  [BLOCKED]
  → r7g (snapshot dry-run)              [BLOCKED]
  → r7h (Admin apply review UI)         [BLOCKED]
  → r8  (Apply + rollback)              [BLOCKED]
        ↓
  r9e3c (read-only) → r9e3h signoff → prod proposal
```

**Not in roadmap:** **r7d** / **r7e** do **not** run local allowlist write smoke, Apply, or rollback execution.
...
|--------|-----------|------|
| **A (recommended)** | **P7.10-r7f** — `GET …/apply-preview` API wiring (read-only; wires [r7d](./P7.10-r7d-admin-apply-preview-payload-builder-closeout.md) builder) | **Next** — still **BLOCKED** on r9e3f2 Grafana + r9e3h signoff for prod-like tiers |
| **B** | **P7.10-r7c** / **r7d** / **r7e** | **DONE** — API design · preview builder · rollback snapshot design |
| **C** | **P7.10-r7g** — Rollback snapshot builder dry-run | After **r7f** |
| **D** | **P7.10-r8** — Apply + rollback implementation | After **r7g** + **r7h** + Gate 12 / Grafana / PM·Ops signoff |
| **E** | **P7.10-r7a** — Rehearsal dry-run payload builder (parallel) | Optional — no DB write |
| **F** | **P7.6-r9e3c** — Read-only prod/staging smoke | **Only when** [r9e3b checklist](../../artifacts/p76/r9e3b/prod-reverify-access-checklist.md) GO |

**Recommendation:** **Option A (r7f)** for engineering next. **Do not start r8** (MatchResult write / rollback execution) until r7f–r7h PASS and Gate 12 / Grafana / signoff close. **r7e is not** local write smoke — it is rollback snapshot **design only** ([r7e doc](./P7.10-r7e-canonical-apply-rollback-snapshot-design.md)).
...
| Apply preview builder (r7d) | [P7.10-r7d](./P7.10-r7d-admin-apply-preview-payload-builder-closeout.md) |
| Rollback snapshot design (r7e) | [P7.10-r7e](./P7.10-r7e-canonical-apply-rollback-snapshot-design.md) |
| Roadmap cleanup (r7e1) | [P7.10-r7c §14](./P7.10-r7c-admin-apply-canonical-promotion-api-design.md) · [P7.6 plan §P7.10](./P7.6-bidirectional-mutual-matching-system-plan.md) |
