...
| 6b | **P7.10-r7i** Admin apply review UI implementation (read-only) | Eng | **NEXT** |
| 7 | **P7.10-r8** Apply + rollback implementation | Eng | **BLOCKED** / **APPLY + ROLLBACK** — first `MatchResult` write |
...
| **r8** | Apply + rollback + migration | **yes** (first write) | **BLOCKED** / **APPLY + ROLLBACK IMPLEMENTATION** |
...
r7b (UX design) → r7c (API design)
  ...
  → r7i (review UI impl, read-only)     [NEXT / READ-ONLY UI IMPLEMENTATION]
  → r8  (Apply + rollback)              [BLOCKED / APPLY + ROLLBACK]
        ↓
...
|----------|-----------|-------|
| **NEXT** | **P7.10-r7i** — [Admin apply review UI](./P7.10-r7h-admin-apply-review-ui-design.md) implementation | **Read-only** · wires r7f · **no** Apply/Rollback/Promote |
| **BLOCKED** | **P7.10-r8** — Apply + rollback | First `MatchResult` write · Gate 12 / Grafana / PM·Ops signoff |
...
**r7f/r7g/r7h DONE ≠ Apply allowed.** Do not start **r8** until **r7i** read-only UI PASS and ops gates close.
