...
| 4 | [P7.10-r7f](./P7.10-r7f-canonical-apply-preview-readonly-api-closeout.md) `GET …/apply-preview` | Eng | **DONE** / **READ-ONLY API PASS** |
| 5 | [P7.10-r7g](./P7.10-r7g-canonical-apply-rollback-snapshot-builder-closeout.md) snapshot dry-run builder | Eng | **DONE** / **DRY-RUN BUILDER PASS** |
| 6 | [P7.10-r7h](./P7.10-r7h-admin-apply-review-ui-design.md) Admin apply review UI design | Eng | **DONE** / **DESIGN PASS** |
| 6b | [P7.10-r7i](./P7.10-r7i-admin-apply-review-ui-readonly-implementation-closeout.md) Admin apply review UI (read-only) | Eng | **DONE** / **READ-ONLY UI IMPLEMENTED** |
| 7 | **P7.10-r8** Apply + rollback implementation | Eng | **NEXT (BLOCKED)** / **APPLY + ROLLBACK** — first `MatchResult` write |
...
|------------|---------------------------|
| `NEED_MORE_PROD_REVERIFY` | **none** for MatchResult mutation — **r8** blocked; **r7f** read-only preview available (Apply still blocked) |
...
| **r7f** | [Apply preview API](./P7.10-r7f-canonical-apply-preview-readonly-api-closeout.md) | **no** (read-only) | **DONE** / **READ-ONLY API PASS** |
...
| **r8** | Apply + rollback + migration | **yes** (first write) | **NEXT (BLOCKED)** / **APPLY + ROLLBACK IMPLEMENTATION** |
...
r7b (UX design) → r7c (API design)
  ...
  → r7g (snapshot dry-run)              [DONE / DRY-RUN BUILDER PASS]
  → r7h (Admin apply review UI design)  [DONE / DESIGN PASS]
  → r7i (review UI impl, read-only)     [DONE / READ-ONLY UI IMPLEMENTED]
  → r8  (Apply + rollback)              [NEXT (BLOCKED) / APPLY + ROLLBACK]
        ↓
  r9e3c (read-only) → r9e3h signoff → prod proposal
```

**Not in roadmap:** **r7d–r7i** do **not** execute Apply/rollback or write `MatchResult` / `finalScore` / `promotionStatus`. **r7i** = read-only UI (no Apply/Rollback/Promote).
...
|----------|-----------|-------|
| **DONE** | **P7.10-r7i** — [Admin apply review UI](./P7.10-r7i-admin-apply-review-ui-readonly-implementation-closeout.md) | **READ-ONLY UI IMPLEMENTED** |
| **NEXT (BLOCKED)** | **P7.10-r8** — Apply + rollback | First `MatchResult` write · Gate 12 / Grafana / PM·Ops signoff |
| **Ops** | [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) · r9e3h | Parallel unblock for r8 |
| **Optional** | **P7.10-r7a** — Rehearsal dry-run payload builder | Parallel track — no DB write |

**r7i DONE ≠ Apply available.** Do not start **r8** until Gate 12 / Grafana / PM·Ops signoff PASS.
