...
**Naming note (r7j1):** [r7b §13](./P7.10-r7b-admin-apply-canonical-write-design.md) uses “r7c” for an older *local write smoke* label. **This document (P7.10-r7c)** is the **HTTP API + promotion state-machine design** only. Canonical Apply: **r7d** DONE · **r7e** DONE · **r7f** READ-ONLY API PASS · **r7g** DRY-RUN BUILDER PASS · **r7h** DESIGN PASS · **r7i** READ-ONLY UI IMPLEMENTED · **r7j** ROLLBACK SNAPSHOT PREVIEW READ-ONLY API PASS · **r8** NEXT (BLOCKED). **r7j DONE ≠ Apply allowed** (no DB row · no MatchResult write).
...
| Staging ops [r9e3g](./P7.6-r9e3g-staging-ops-evidence-rollup-and-gate12-review.md) | **Partial** | GET matrix + watch **done**; Grafana **`PASS_GRAFANA_PHYSICAL_IMPORT_AND_TEST_ALERT`** ([r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md)); **PM/Ops signoff pending** |
...
| 7c | [r7j](./P7.10-r7j-rollback-snapshot-preview-readonly-api-closeout.md) `GET …/rollback-snapshot-preview` | Eng — **DONE** / **READ-ONLY API PASS** |
...
| 11 | Monitoring P0 routes + **physical** Grafana ([r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md)) | Ops — **PASS** (r9e3f2); **PM/Ops signoff still pending** |
...
| [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) | `PASS_GRAFANA_PHYSICAL_IMPORT_AND_TEST_ALERT` |
...
| **r7j** | [Rollback snapshot preview API](./P7.10-r7j-rollback-snapshot-preview-readonly-api-closeout.md) — wires r7g | **no** (read-only HTTP; no snapshot DB row) | **DONE** / **ROLLBACK SNAPSHOT PREVIEW READ-ONLY API PASS** |
...
r7c (API design)                              [DONE]
  ...
  → r7j (GET rollback-snapshot-preview)      [DONE / ROLLBACK SNAPSHOT PREVIEW READ-ONLY API PASS]
  → r8  (Apply + rollback — MatchResult write) [NEXT (BLOCKED) / APPLY + ROLLBACK]
  → staging promotion smoke → r9e3h → prod proposal
```

**Not in roadmap:** r7d–r7j **do not** execute Apply/rollback or write `MatchResult` / `finalScore` / `promotionStatus`. **r7j** does not create rollback snapshot DB rows. **r7i** ships read-only UI only (no Apply/Rollback/Promote). **r8** is the first milestone that may write `MatchResult` (still **BLOCKED** until Gate 12 / Grafana / signoff).
...
| **DONE** | **P7.10-r7j** — [Rollback snapshot preview API](./P7.10-r7j-rollback-snapshot-preview-readonly-api-closeout.md) | **READ-ONLY API PASS** · dry-run snapshot · no DB row |
| **DONE** | **P7.10-r8a** — [Controlled preflight](./P7.10-r8a-canonical-apply-rollback-controlled-implementation-preflight-closeout.md) | **`PASS_R8A_CONTROLLED_IMPLEMENTATION_PREFLIGHT`** · boundary only |
| **NEXT** | **P7.10-r8b** — Rollback snapshot migration design | design only · no migrate |
| **Ops** | [r9e3h](./P7.6-r9e3h-final-gate12-pm-ops-signoff.md) · [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) | Gate 12 PASS · Grafana PASS |

**r7j DONE ≠ Apply available.** **r8a DONE ≠ Apply enabled.** No canonical promotion before **r8d** smoke + ops gates. percent / prod rollout / legacy removal remain **BLOCKED**.
