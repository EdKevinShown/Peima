...
**Naming note (r7h2):** [r7b §13](./P7.10-r7b-admin-apply-canonical-write-design.md) uses “r7c” for an older *local write smoke* label. **This document (P7.10-r7c)** is the **HTTP API + promotion state-machine design** only. Canonical Apply ids: **r7d** DONE · **r7e** DONE (design) · **r7f** **READ-ONLY API PASS** · **r7g** **DRY-RUN BUILDER PASS** · **r7h** **DESIGN PASS** · **r7i** **NEXT** (read-only UI) · **r8** **BLOCKED** (Apply + rollback). **r7f/r7g/r7h DONE ≠ Apply allowed.**
...
| 7b | **r7i** Admin apply review UI implementation (read-only) | Eng — **NEXT** / **READ-ONLY UI IMPLEMENTATION** |
...
| **r7i** | Admin apply review UI implementation (**read-only**; no Apply/Rollback/Promote) | **no** | **NEXT** / **READ-ONLY UI IMPLEMENTATION** |
...
r7c (API design)                              [DONE]
  ...
  → r7i (review UI impl, read-only only)       [NEXT / READ-ONLY UI IMPLEMENTATION]
  → r8  (Apply + rollback — MatchResult write) [BLOCKED / APPLY + ROLLBACK]
  → staging promotion smoke → r9e3h → prod proposal
```

**Not in roadmap:** r7d–r7h **do not** implement Apply, rollback execution, or `MatchResult` / `finalScore` / `promotionStatus` writes. **r7f/r7g/r7h DONE ≠ Apply allowed.** **r7i** must not add Apply / Rollback / Promote controls. **r8** is the first milestone that may write `MatchResult`.
...
|----------|-----------|-------|
| **NEXT** | **P7.10-r7i** — [Admin apply review UI](./P7.10-r7h-admin-apply-review-ui-design.md) implementation | **Read-only** · wires r7f · optional `GET …/rollback-snapshot-preview` · **no** Apply/Rollback/Promote buttons |
| **BLOCKED** | **P7.10-r8** — Apply + rollback | Requires Gate 12 / Grafana / PM·Ops signoff · first `MatchResult` write |
| **Ops** | [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) · [r9e3h](./P7.6-r9e3g-staging-ops-evidence-rollup-and-gate12-review.md) | Grafana import + final signoff pending |

**r7h design pass does not authorize Apply.** Preview may show `canApply=true` in tests only — production Apply remains **BLOCKED** until **r8** + ops gates PASS.
