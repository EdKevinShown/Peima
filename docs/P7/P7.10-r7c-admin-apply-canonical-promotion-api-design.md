...
**Naming note:** [r7b §13](./P7.10-r7b-admin-apply-canonical-write-design.md) uses “r7c” for an older *local write smoke* label. **This document (P7.10-r7c)** is the **HTTP API + promotion state-machine design** only. Canonical Apply implementation ids are fixed: **r7d** preview builder (pure, **DONE**) · **r7e** rollback snapshot design (**DONE**, docs only) · **r7f** `GET …/apply-preview` (read-only) · **r7g** snapshot builder dry-run · **r7h** Admin UI design · **r8** Apply + rollback (**BLOCKED** until Gate 12 / Grafana / signoff). **Do not** use r7d/r7e for MatchResult writes or rollback execution.
...
| 5 | **r7f** `GET …/apply-preview` API wiring (**read-only**) | Eng — **BLOCKED** |
| 6 | **r7g** rollback snapshot builder dry-run | Eng — **BLOCKED** |
| 7 | **r7h** Admin apply review UI design | Eng — **BLOCKED** |
| 8 | **r8** Apply + rollback implementation + migration | Eng — **BLOCKED** |
...
| **r7f** | Nest `GET …/apply-preview` — wires r7d builder | **no** (read-only HTTP) | **BLOCKED** |
| **r7g** | Rollback snapshot builder dry-run (`P76CanonicalApplyRollbackSnapshotV1`) | **no** until r8 migration | **BLOCKED** |
| **r7h** | Admin apply review UI design (hidden flags) | **no** | **BLOCKED** |
| **r8** | Apply + rollback + Option B snapshot table migration | **yes** (first write milestone) | **BLOCKED** until Gate 12 / Grafana / r9e3h |
| — | Staging promotion smoke | — | **BLOCKED** |
| — | Production cohort proposal | — | **BLOCKED** |

```text
r7c (API design)
  → r7d (preview builder, pure no-write)     [DONE]
  → r7e (rollback snapshot design)           [DONE]
  → r7f (GET apply-preview, read-only)       [BLOCKED]
  → r7g (snapshot builder dry-run)         [BLOCKED]
  → r7h (Admin apply review UI design)       [BLOCKED]
  → r8  (Apply + rollback implementation)  [BLOCKED]
  → staging promotion smoke → r9e3h signoff → prod proposal
```

**Not in roadmap:** r7d/r7e **do not** implement Apply, rollback execution, or `MatchResult` promotion.
...
1. **P7.10-r7f** — Nest `GET …/apply-preview` (wire [r7d](./P7.10-r7d-admin-apply-preview-payload-builder-closeout.md) builder).  
2. **P7.10-r7g** — Rollback snapshot pure builder (`P76CanonicalApplyRollbackSnapshotV1`).  
3. **P7.10-r8** — Apply + rollback per this doc + [r7e](./P7.10-r7e-canonical-apply-rollback-snapshot-design.md).  
4. **Ops** — Close [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) Grafana gap.  
5. **P7.6-r9e3h** — Final PM/Ops Gate 12 signoff (**not** in r7c scope).
