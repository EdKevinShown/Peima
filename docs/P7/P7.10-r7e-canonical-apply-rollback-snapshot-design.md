...
| **r8** | Apply + rollback + snapshot table migration | **yes** (first write) | **NEXT (BLOCKED)** / **APPLY + ROLLBACK IMPLEMENTATION** |

```text
r7d [DONE] → r7e [DONE] → r7f [READ-ONLY API PASS] → r7g [DRY-RUN BUILDER PASS]
  → r7h [DESIGN PASS] → r7i [READ-ONLY UI IMPLEMENTED] → r8 [NEXT (BLOCKED)] → staging → r9e3h
```
...
| **NEXT (BLOCKED)** | **P7.10-r8** — Apply + rollback |
