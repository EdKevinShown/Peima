...
| **r7f** | `GET …/apply-preview` HTTP (r7d payload) | **no** | **BLOCKED** |
| **r7g** | Snapshot builder dry-run (`mode: dry_run`) | **no** | **BLOCKED** |
| **r7h** | Admin apply review UI design | **no** | **BLOCKED** |
| **r8** | Apply + rollback + snapshot table migration | **yes** (first write) | **BLOCKED** until Gate 12 / Grafana / r9e3h |

```text
r7d (preview, no-write) → r7e (snapshot design) → r7f → r7g → r7h → r8 → staging → r9e3h
```

**r7e does not:** execute rollback · mint tokens in DB · promote `MatchResult`.

## 14. 下一步

**P7.10-r7f** — Nest `GET …/apply-preview` wiring (read-only).

**Ops (parallel):** Close [r9e3f2](./P7.6-r9e3f2-grafana-physical-import-and-test-alert-evidence-closeout.md) → [r9e3h](./P7.6-r9e3g-staging-ops-evidence-rollup-and-gate12-review.md) final signoff.
