/**
 * questionnaire.scorer 受控回归（G-q25 / q01–q30 全 canonical=30；q06 的 5D·5E）。
...
describe("questionnaire.scorer regression (G-q25 / 30 canonical / q06-5E)", () => {
  ...
  it("G-q25-only", () => {
    ...
    expect(out.confidence).toBe(1 / 30);
  ...
  /** G-two-canonical-q01-q06：仅答 q01+q06，confidence 分子=2、分母=30 */
  it("G-two-canonical-q01-q06", () => {
    const out = scoreQuestionnaireG1r([
      { questionKey: "q01", answerValue: "A" },
      { questionKey: "q06", answerValue: "A" },
    ]);

    expect(out.confidence).toBe(2 / 30);
    expectAllNullExcept(out, {
  ...
  /** G-q01-plus-q25：q01 与 q25 一并参与聚合（confidence 分母=30） */
  it("G-q01-plus-q25", () => {
  ...
  it("G-q13-A-vs-D-axis-diff", () => {
    ...
    expect(outD.confidence).toBe(1 / 30);
    expect(outA.confidence).toBe(1 / 30);
