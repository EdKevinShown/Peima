...
The server overwrites viewerUserId, poolId, candidateAUserId, and candidateBUserId from the job shortlist (do not invent ids; echoing wrong ids is ignored).
...
decisionScoreA/B in [0,100]. decisionConfidence is required: output a plain JSON number strictly between 0 and 1 inclusive (e.g. 0.82) — not a string, not a percentage, not an object; the server may coerce a narrow set of mistakes (numeric strings in [0,1], or { confidence: number }) when the top-level number is missing or malformed, but you must still aim for a top-level number in [0,1].
For dimensions.* and the same six keys on candidateA / candidateB — conversationFit, emotionalSafety, conflictRepair, progressionFit, longTermFit, riskControl — each must be a plain JSON number in [0,1] (not strings, not 0–100). Prefer these six numbers on each candidate object; the server may fill missing candidate axes from root dimensions.* or unwrap nested maps only as a fallback — still output correct flat numbers yourself. Do not wrap these scores in objects for the final shape; put prose only in reasonSummary and decisionReason. The server may unwrap plain { score } / { value } / { rating } with numeric 0–1 when hoisting from nested maps; do not use 0–100 scales for these fields.
per-candidate reasonSummary max ~60 Chinese characters or ~80 English words; decisionReason max ~120 Chinese characters.
fallbackUsed must be a JSON boolean true or false only — not the strings "true"/"false", not 0/1 (the server may coerce common string/number mistakes, but prefer a real boolean).
candidateA.strongRisk and candidateB.strongRisk must each be JSON booleans true or false only — not strings, not 0/1 numbers (the server may coerce "true"/"false", 0/1, or { value: boolean } in limited cases; prefer real booleans).
Each of candidateA and candidateB must include suggestedAction as exactly one of: maintain, slow_down, stop_or_step_back (ASCII snake_case strings). Each must include progressionWindow as exactly one of: open, weak_open, closed. Put these on the candidate object itself; the server may hoist strict literals from nested action/recommendation/guidance/decision/candidate/profile/dimensions objects when the top-level field is missing — never from natural-language labels or Chinese prose in reasonSummary/decisionReason.
