...
export type MatchExplanationAiChatFailureKind =
  | "disabled"
  | "missing_api_key"
  | "timeout"
  | "network"
...
export type MatchExplanationAiChatResult =
  | { ok: true; content: string }
  | {
      ok: false;
      kind: MatchExplanationAiChatFailureKind;
      status?: number;
      detail?: string;
    };

@Injectable()
export class MatchExplanationAiChatCompletionsClient {
  constructor(private readonly aiConfig: MatchExplanationAiConfigService) {}

  async complete(
    system: string,
    user: string,
  ): Promise<MatchExplanationAiChatResult> {
