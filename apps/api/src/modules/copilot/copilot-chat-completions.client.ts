...
export type CopilotChatResult =
  ...
  | { ok: false; kind: CopilotChatFailureKind; status?: number; detail?: string };

@Injectable()
export class CopilotChatCompletionsClient {
  constructor(private readonly aiConfig: CopilotAiConfigService) {}

  /**
   * OpenAI-compatible Chat Completions (POST /v1/chat/completions).
   * Other providers work if they expose the same JSON shape.
   */
  async complete(system: string, user: string): Promise<CopilotChatResult> {
